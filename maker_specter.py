...
spec = [('width', float64), ('height', float64), ('distance', float64), ('azimuth', float64), ('orientation', float64)]
...
# log_file = pd.read_csv("stations_data.csv", delimiter=",")
def anim(i):
    # print(i)
    im.set_array(radon_array[i])
    return [im]
...
for name in stations:
    ...
    for t in range(full_time):
        ...
        back_cartesian_3d_0 = speed_bilinear_interpol(back_polar, area_mask_div, area_mask_mod)

        # radon_argmax[t] = np.argmax(np.max(radon(mask_circle * back_cartesian_3d_0), axis=0))

        radon_array[t] = radon(mask_circle * back_cartesian_3d_0)
    ...
    #
    # log_file.to_csv("stations_data.csv", index=False)
    ...
    im = axs.imshow(radon_array[0], aspect=0.25, extent=[0, 180, 0, 720], origin='lower', cmap='gnuplot',
                    interpolation='None')
    animation = FuncAnimation(fig, anim, frames=full_time, interval=10000, blit=True)
    writer = PillowWriter(fps=2.5)
    axs.set_xlabel(r"Угол $\theta$, град")
    axs.set_ylabel(r"Расстояние $\rho$, м")
    animation.save("radon_" + name[-7:-3] + ".gif", writer=writer)
